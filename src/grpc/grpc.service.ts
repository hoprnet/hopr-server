...
import { toGrpcState } from './grpc.utils'
...
export class GrpcService {
  ...
  async getChannels(): Promise<GetChannelsResponse.AsObject> {
    const channels = await this.coreService.getChannels()

    const { formattedChannels, totalBalance, myTotalBalance } = channels.reduce(
      (result, channel) => {
        result.totalBalance + BigInt(channel.totalBalance)
        result.myTotalBalance + BigInt(channel.myBalance)
        result.formattedChannels.push({
          state: toGrpcState(channel.state),
      ...
      {
        formattedChannels: [] as GetChannelsResponse.AsObject['channelsList'],
        totalBalance: BigInt(0),
        myTotalBalance: BigInt(0),
      },
    )
  ...
  async getChannelData({ channelId }: GetChannelDataRequest.AsObject): Promise<GetChannelDataResponse.AsObject> {
    ...
    return this.coreService.getChannelData(channelId).then((res) => ({
      // here we add 1 because we reserve state 0 for an UNKNOWN state
      state: toGrpcState(res.state),
